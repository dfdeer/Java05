...
public class UserAccountmanager {
	...
	public boolean isUserIdDuplicated(String inUserId){
			for (UserAccount user : userList) {
				if (user.getUserId() == inUserId) return true;
			}
			return false;
		}
	...
	public boolean isPhoneNumberDuplicated(String inPhoneNumber) {
		for (UserAccount user : userList) {
			if (user.getPhoneNumber() == inPhoneNumber) return true;
		}
		return false;
	}
	...
	public boolean createUserAccount(String inUserId, String inPassword, String inName, String inPhoneNumber) {
		if (isUserIdDuplicated(inUserId)) return false;
		if (isPhoneNumberDuplicated(inPhoneNumber)) return false;
	...
	public boolean login(String inUserId, String inPassword) {
		for (UserAccount user : userList) {
			if (user.getUserId() == inUserId && user.getPassword() == inPassword) {
				currentUser = user;
				return true;
			}
		}
